...
const InputText: React.FC<
  InputProps & { iconEnd?: React.ReactNode; iconStart?: boolean; field: ControllerRenderProps }
> = ({ label, iconEnd = null, placeholder, useFormProps, field }) => {
  // const form = useForm();
  // return (
  //   <Form {...useFormProps}>
  //     <form>
  ...
  return (
    <FormItem>
      ...
      <FormControl>
        <div className="text-sm font- relative flex flex-col gap-2  ">
          <Input placeholder={placeholder} label={label} useFormProps={useFormProps} {...field} />
          {iconEnd ? (
            <span className="absolute top-0 right-0 rounded-e-md w-10 text-gray-200 flex focus-visible:text-gray-400 items-center justify-center border-l border-gray-200 h-full">
              {iconEnd}
            </span>
